import { faBell, faHome, faMessage, faSignOut, faUserAlt } from '@fortawesome/free-solid-svg-icons'
...
function Index() {
  ...
  const [active, setActive] = useState(menus.findIndex((x) => x.link === pathname))
    ...
    <div className="md:hidden flex justify-center fixed bottom-0 w-full bg-white left-0 z-20 h-16">
      <div className="flex text-gray-600 relative">
        ...
        {menus.map((menu, index) => (
          ...
          >
            <span className={`text-xl z-20 duration-500 ${index === active && '-mt-6 text-white'}`}>
